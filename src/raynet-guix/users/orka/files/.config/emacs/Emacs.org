...
#+begin_src emacs-lisp :tangle modules/dw-writing.el
...
#+begin_src emacs-lisp :tangle modules/dw-workflow.el
  ...
  (defvar dw/base-agenda-files '("inbox.org" "agenda.org")
  ...
  (setq org-todo-keywords
        '((sequence
           "TODO(t)" "PLANNING(p)" "IN-PROGRESS(s)" "VERIFYING(v)" "BLOCKED(b)" "IDEA(i)"
           "|"
           "DONE(d)" "DELIGATED(e)" "WONT-DO(n)")))
...
I'm leveraging the =projectile= package to manage the tasks I perform around the projects I'm working on.

#+begin_src emacs-lisp :tangle modules/dw-develop.el
  ...
  (defun dw/project-compilation-buffer-name-function (name-of-mode)
    (format "*compilation: %s*" (projectile-project-name)))

  (setq compilation-buffer-name-function 'dw/project-compilation-buffer-name-function)
...
#+begin_src emacs-lisp :tangle modules/dw-present.el
...
#+begin_src emacs-lisp :tangle modules/dw-chat.el
...
#+begin_src emacs-lisp :tangle modules/dw-0x0.el
...
#+begin_src emacs-lisp :tangle modules/dw-telegram.el
...
#+begin_src emacs-lisp :tangle modules/dw-desktop.el
...
#+begin_src emacs-lisp :tangle modules/dw-auth.el
...
#+begin_src emacs-lisp :tangle modules/dw-mail.el
...
#+begin_src emacs-lisp :tangle modules/dw-system.el
  ...
  (defun dw/reconfigure-home ()
    (interactive)
    (let ((tangle-script (expand-file-name "tangle.sh" dw/config-directory)))
      (async-shell-command tangle-script "*reconfigure-home*")))

  ;; Unbind C-c G if it is already bound to something else (like org-roam-graph)
  (keymap-global-unset "C-c G" t)
...
#+begin_src emacs-lisp :tangle modules/dw-ai.el

  (use-package aider
    :ensure nil
    :config
    (setq aider-args '("--model" "gemini" "--no-auto-accept-architect"))
    (setenv "GEMINI_API_KEY" (string-trim (shell-command-to-string "pass data/google/gemini_api.key")))
    (global-set-key (kbd "C-c H") 'aider-transient-menu))

  (provide 'dw-ai)

#+end_src

** =dw-meow= - Meow Editing

#+begin_src emacs-lisp :tangle modules/dw-meow.el
  ...
  (defun my/meow-jump-to-pair ()
    "Jump to matching pair, adapted from dotfiles."
    (interactive)
    (let ((pos (point)))
      (condition-case nil
          (cond
           ...
           ((and (char-before) (string-match "\\s\)" (char-to-string (char-before)))) (backward-list 1))
           ((and (char-before) (string-match "\\s\(" (char-to-string (char-before)))) (backward-char 1) (forward-list 1) (backward-char 1))
           (t
            (save-excursion
  ...
  (use-package meow
    ...
    ;; Two-char escape sequence (jk) from dotfiles
    ...
    ;; Define a prefix map for notes
    (define-prefix-command 'dw/meow-notes-map)
    ...
    (meow-normal-define-key
     ...
     '("?" . consult-ripgrep-with-last-regex)
     '("M-f" . find-grep-dired)
    ...
    (with-eval-after-load 'projectile
      ...
      (keymap-set projectile-command-map "b" #'consult-project-buffer)
      (keymap-set projectile-command-map "/" #'consult-ripgrep)
      (when (boundp 'meow-leader-keymap)
        (which-key-add-keymap-based-replacements meow-leader-keymap
          "p" '("project" . projectile-command-map))))

    ;; Populate the notes menu
    (keymap-set dw/meow-notes-map "a" #'org-agenda)
    (keymap-set dw/meow-notes-map "c" #'org-capture)
    (with-eval-after-load 'org-roam
      (keymap-set dw/meow-notes-map "f" #'org-roam-node-find)
      (keymap-set dw/meow-notes-map "i" #'org-roam-node-insert)
      (keymap-set dw/meow-notes-map "l" #'org-roam-buffer-toggle)
      (keymap-set dw/meow-notes-map "p" #'org-roam-capture)
      (keymap-set dw/meow-notes-map "g" #'org-roam-graph)
      (keymap-set dw/meow-notes-map "d" #'org-roam-dailies-capture-today)
      (keymap-set dw/meow-notes-map "t" #'org-roam-dailies-goto-date)))  (provide 'dw-meow)

#+end_src

** =dw-extras= - Convenient Extra Packages

#+begin_src emacs-lisp :tangle modules/dw-extras.el
...
#+begin_src emacs-lisp :tangle systems/ser8.el
...
#+begin_src emacs-lisp :tangle systems/phantom.el
...
#+begin_src emacs-lisp :tangle systems/daviwil-x1e.el
...
#+begin_src emacs-lisp :tangle systems/cereal.el
