...
** =dw-helix= - Helix-like Editing

#+begin_src emacs-lisp :tangle modules/dw-helix.el
  ...
  (use-package helix
    ...
    (defun helix-define-key (map-type key command)
      "Define a key in a Helix map. MAP-TYPE is 'space for helix-space-map."
      (let ((map (cond ((eq map-type 'space) helix-space-map)
                       (t nil))))
        (if (and map (stringp (car-safe command)))
            (define-key map key command)
          (define-key map key command))))

    (with-eval-after-load 'which-key
      (which-key-add-keymap-based-replacements helix-normal-state-keymap
        "SPC" '("Space Menu" . helix-space-map))

      ;; Notes (n) sub-menu
      (define-prefix-command 'dw/helix-notes-map)
      (helix-define-key 'space "n" '("notes" . dw/helix-notes-map))

      (helix-define-key 'space "f" '("Files" . project-find-file))
      (helix-define-key 'space "b" '("Buffers" . consult-buffer))

      (define-key dw/helix-notes-map "a" '("Org Agenda" . org-agenda))

      (with-eval-after-load 'org-roam
        (define-key dw/helix-notes-map "f" '("Node Find" . org-roam-node-find))
        (define-key dw/helix-notes-map "i" '("Node Insert" . org-roam-node-insert))
        (define-key dw/helix-notes-map "l" '("Buffer Toggle" . org-roam-buffer-toggle))
        (define-key dw/helix-notes-map "c" '("Capture" . org-roam-capture))
        (define-key dw/helix-notes-map "d" '("Roam Dailies" . org-roam-dailies-capture-today)))

      (define-key dw/helix-notes-map "t" '("Todo List" . (lambda () (interactive) (org-agenda nil "t")))))
    ;; Fix ESC in terminal for helix-mode
    (define-key helix-insert-state-keymap [escape] #'helix-insert-exit)
    (define-key helix-insert-state-keymap (kbd "ESC") #'helix-insert-exit))
