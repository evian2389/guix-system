...
#+END_SRC
#+BEGIN_QUOTE md
| **4.2** | **2026-01-27** | **Feedback 반영: 항목 이동/삭제/추가, 용어 변경** |
...
language-servers = ["marksman", "markdown-oxide", "iwe"]
...
#+NAME: script-git-commit-msg-hook.sh
#+auto_tangle: nil
#+BEGIN_SRC git-commit-msg-hook.sh :tangle ~/dotfiles/.local/bin/git-commit-msg-hook.sh :mkdirp yes
#!bash
gitdir=$(git rev-parse --git-dir);   scp -O -p -P 29440 jongho3.lee@vgit.lge.com:hooks/commit-msg  ${gitdir}/hooks/
#+END_SRC
** git-tag-add.sh
#+NAME: script-git-tag-add.sh
#+auto_tangle: nil
#+BEGIN_SRC git-tag-add.sh :tangle ~/dotfiles/.local/bin/git-tag-add.sh :mkdirp yes
set -x
...
** git-tag-get-info.sh
#+NAME: script-git-tag-get-info.sh
#+auto_tangle: nil
#+BEGIN_SRC git-tag-get-info.sh :tangle ~/dotfiles/.local/bin/git-tag-get-info.sh :mkdirp yes
set -x

TAG2=$1
TAG2=$2
echo "############"
TAG1=${TAG1:-$(git tag --merged HEAD --sort=-creatordate | sed -n '2p')}

TAG2=${TAG2:-$(git tag --merged HEAD --sort=-creatordate | sed -n '1p')}
git show-ref ${TAG2}
echo "############"
git log --oneline --no-decorate ${TAG1}..${TAG2}
echo "############"
