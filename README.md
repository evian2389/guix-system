...
*   **System Name:** `ser8`
*   **User Name:** `orka`
*   **Disk Setup:** Btrfs filesystem and zstd compressed multiple subvolumes (`@`, `@home`, `@boot`, `@var`, etc.).
...
*   `Makefile`: Automates common tasks like initial setup, configuration, and channel updates. Using stow fot manage dotfiles.
*   `scripts/prepare-system.sh`: A script to prepare the disk with Btrfs and zstd before initial installation.
*   `channels/`:
    ...
    *   `users/`: User-specific configurations.
        *   `orka/`:
            ...
            *   `files/`: Directory for `orka`'s managed dotfiles, applied recursively to `~/`.
                * emacs/ : Directory for managing emacs configs and dotfiles configs. Using tangle.sh to apply changes to users/orka/files.
                           Makefile in root directory of this project will use stow to link users/orka/files/ to '~/'.
...
3.  **Prepare the Disk (Btrfs)**:
    *   Enter a Guix shell with the necessary tools:
        ```bash
        guix shell parted btrfs-progs make git --
        ```
