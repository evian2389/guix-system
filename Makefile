...
install-system: guix-pull channel-update
	...
	sudo $(GUIX_TM) system init -L $(CONFIG_DIR) $(CONFIG_DIR)/systems/$(DEFAULT_SYSTEM)/configuration.scm /mnt

reconfigure-system:
	@echo "Reconfiguring Guix System for $(DEFAULT_SYSTEM)..."
	sudo $(GUIX_TM) system reconfigure -L $(CONFIG_DIR) $(CONFIG_DIR)/systems/$(DEFAULT_SYSTEM)/configuration.scm
